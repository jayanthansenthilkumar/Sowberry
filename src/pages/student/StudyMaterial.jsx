...
const StudyMaterial = () => {
  ...
  const typeColors = { pdf: 'bg-red-500/15 text-red-400', video: 'bg-blue-500/15 text-blue-400', document: 'bg-green-500/15 text-green-400', link: 'bg-[#d4a574]/15 text-[#d4a574]', presentation: 'bg-amber-500/15 text-amber-400', notes: 'bg-teal-500/15 text-teal-400' };
        ...
        <div className="space-y-4 h-full">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              <button onClick={() => setViewingMaterial(null)} className="px-3 py-2 rounded-xl bg-[#222] border border-[#333] text-[#aaa] text-sm hover:bg-[#2a2a2a] transition-colors flex items-center gap-1.5 flex-shrink-0">
                <i className="ri-arrow-left-line"></i>Back
              </button>
              <div className="min-w-0">
                <h2 className="font-bold text-[#e8e8e8] truncate">{viewingMaterial.title}</h2>
                <p className="text-xs text-[#666]">{viewingMaterial.courseName || 'General'}</p>
              </div>
            </div>
            <a href={viewingMaterial.url} target="_blank" rel="noopener noreferrer" className="px-3 py-2 rounded-xl bg-[#d4a574] text-[#1a1a1a] font-semibold text-sm hover:bg-[#c4956a] transition-colors flex items-center gap-1.5 flex-shrink-0">
              <i className="ri-external-link-line"></i>Open in Tab
            </a>
          </div>
          <div className="bg-[#1a1a1a] rounded-2xl border border-[#2a2a2a] overflow-hidden" style={{ height: 'calc(100vh - 200px)' }}>
            <iframe
    ...
    <DashboardLayout pageTitle="Study Material" role="student">
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-[#e8e8e8]">Study Materials</h1>
            <p className="text-sm text-[#666] mt-1">{materials.length} resources available</p>
          </div>
          <div className="relative w-full sm:w-64">
            ...
            <input type="text" placeholder="Search materials..." value={search} onChange={e => setSearch(e.target.value)}
              className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-[#222] border border-[#333] focus:border-[#d4a574] outline-none text-sm text-[#e8e8e8] placeholder-[#555]" />
          </div>
        </div>

        <div className="flex gap-2 flex-wrap">
          {types.map(t => (
            <button key={t} onClick={() => setFilterType(t)} className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-colors ${filterType === t ? 'bg-[#d4a574] text-[#1a1a1a]' : 'bg-[#222] text-[#888] border border-[#333] hover:border-[#555]'}`}>
        ...
        filtered.length === 0 ? <div className="text-center py-20 text-[#555]"><i className="ri-file-text-line text-4xl mb-3 block"></i><p>{search ? 'No matching materials found' : 'No study materials available'}</p></div> :
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((m, i) => {
            const mType = m.type || 'document';
            return (
              <div key={m.id || i} onClick={() => openMaterial(m)} className="bg-[#1a1a1a] rounded-2xl p-5 border border-[#2a2a2a] hover:border-[#d4a574]/30 hover:shadow-lg hover:shadow-[#d4a574]/5 transition-all cursor-pointer group">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-[#e8e8e8] text-sm truncate group-hover:text-[#d4a574] transition-colors">{m.title}</h3>
                    <p className="text-[11px] text-[#555] mt-0.5">{m.courseName || 'General'}</p>
                  </div>
                </div>
                {m.description && <p className="text-xs text-[#666] line-clamp-2 mb-3">{m.description}</p>}
                <div className="flex items-center justify-between">
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${typeColors[mType] || typeColors.document}`}>{mType}</span>
                  <span className="text-xs text-[#d4a574] flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><i className="ri-eye-line"></i>View</span>
                </div>
              </div>
            );
          })}
