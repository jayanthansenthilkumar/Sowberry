...
const CoursesOverview = () => {
  ...
  const addSubject = async () => {
    const { value } = await Swal.fire({ title: 'Add Subject', html: '<input id="s-title" class="swal2-input" placeholder="Subject Title"><input id="s-code" class="swal2-input" placeholder="Subject Code (optional)"><textarea id="s-desc" class="swal2-textarea" placeholder="Description (optional)"></textarea>', showCancelButton: true, confirmButtonColor: '#7c3aed', preConfirm: () => { const t = document.getElementById('s-title').value; if (!t) { Swal.showValidationMessage('Title required'); return; } return { title: t, code: document.getElementById('s-code').value, description: document.getElementById('s-desc').value }; } });
  ...
  const addTopic = async (subjectId) => {
    const { value } = await Swal.fire({ title: 'Add Topic', html: '<input id="t-title" class="swal2-input" placeholder="Topic Title"><textarea id="t-desc" class="swal2-textarea" placeholder="Description (optional)"></textarea>', showCancelButton: true, confirmButtonColor: '#7c3aed', preConfirm: () => { const t = document.getElementById('t-title').value; if (!t) { Swal.showValidationMessage('Title required'); return; } return { title: t, description: document.getElementById('t-desc').value }; } });
  ...
  const statusBadge = (s) => ({ pending: 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300', active: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300', rejected: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300', draft: 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300', inactive: 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400' })[s] || 'bg-gray-100 text-gray-800';
    ...
    return (
      <AdminLayout>
        <div className="max-w-5xl mx-auto">
          <button onClick={() => setDetailCourse(null)} className="flex items-center gap-2 text-purple-600 hover:text-purple-800 mb-4 font-medium">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg">
              ...
              <div className="p-6 border-b dark:border-gray-700 flex items-start justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-2 text-sm text-gray-500 dark:text-gray-400">
                    {detailCourse.courseCode && <span className="bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 px-2 py-0.5 rounded text-xs font-mono">{detailCourse.courseCode}</span>}
                  ...
                  {detailCourse.description && <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">{detailCourse.description}</p>}
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => openEdit(detailCourse)} className="px-3 py-1.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-lg text-sm font-medium hover:bg-blue-200 transition"><i className="ri-edit-line mr-1"></i>Edit</button>
              ...
              <div className="border-b dark:border-gray-700 flex">
                {['subjects', 'content'].map(t => (
                  <button key={t} onClick={() => setActiveTab(t)} className={`px-6 py-3 text-sm font-medium border-b-2 transition ${activeTab === t ? 'border-purple-600 text-purple-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}>
                ...
                {activeTab === 'subjects' && (
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="font-semibold text-gray-900 dark:text-white">Subjects & Topics</h3>
                      <button onClick={addSubject} className="px-4 py-2 bg-purple-600 text-white text-sm rounded-lg hover:bg-purple-700 transition"><i className="ri-add-line mr-1"></i>Add Subject</button>
                    </div>
                    {(!detailCourse.subjects || detailCourse.subjects.length === 0) ? (
                      ...
                      <div className="space-y-4">
                        {detailCourse.subjects.map(sub => (
                          <div key={sub.id} className="border dark:border-gray-700 rounded-xl overflow-hidden">
                            <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50">
                              <div>
                                <h4 className="font-semibold text-gray-900 dark:text-white">{sub.title}</h4>
                              ...
                              <div className="divide-y dark:divide-gray-700">
                                {sub.topics.map(t => (
                                  <div key={t.id} className="flex items-center justify-between p-3 px-6">
                                    <span className="text-sm text-gray-700 dark:text-gray-300"><i className="ri-bookmark-line mr-2 text-purple-400"></i>{t.title}</span>
                ...
                {activeTab === 'content' && (
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button onClick={addContent} className="px-4 py-2 bg-purple-600 text-white text-sm rounded-lg hover:bg-purple-700 transition"><i className="ri-add-line mr-1"></i>Add Content</button>
                    </div>
                    {contentLoading ? (
                      <div className="text-center py-8"><i className="ri-loader-4-line animate-spin text-2xl text-purple-500"></i></div>
                    ) : contentList.length === 0 ? (
                      <p className="text-center text-gray-500 py-8">No content yet. Add videos, documents, or text content.</p>
                    ) : (
                      <div className="space-y-3">
                        {contentList.map(c => (
                          <div key={c.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg group">
                            <div>
                              <p className="font-medium text-gray-900 dark:text-white">{c.title}</p>
                              <div className="flex items-center gap-3 text-xs text-gray-500 mt-1">
                                <span className="bg-purple-100 dark:bg-purple-900/30 text-purple-600 px-2 py-0.5 rounded">{c.contentType}</span>
    ...
    <AdminLayout>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Courses Overview</h1>
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Manage all courses, subjects, and content</p>
          </div>
          <button onClick={openCreate} className="px-5 py-2.5 bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition shadow-lg hover:shadow-xl font-medium">
          ...
          {[
            ...
            <div key={i} className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-lg bg-${s.color}-100 dark:bg-${s.color}-900/30 flex items-center justify-center`}>
        ...
        {pending.length > 0 && (
          <div className="mb-6 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl flex items-center gap-3">
            ...
            <span className="text-amber-800 dark:text-amber-300 font-medium">{pending.length} course{pending.length > 1 ? 's' : ''} awaiting approval</span>
            <button onClick={() => setStatusFilter('pending')} className="ml-auto px-3 py-1.5 rounded-lg bg-amber-100 dark:bg-amber-900/30 text-amber-700 text-xs font-medium hover:bg-amber-200">Review Now</button>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-xl">
            <i className="ri-book-open-line text-5xl text-gray-300 dark:text-gray-600 mb-3 block"></i>
            <h3 className="text-lg font-semibold text-gray-600 dark:text-gray-400">No courses found</h3>
            <p className="text-gray-400 text-sm mt-1">Click "Create Course" to add your first course</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map(c => (
              <div key={c.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-lg transition overflow-hidden group border border-transparent hover:border-purple-200 dark:hover:border-purple-800">
                <div className="p-5">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 mb-3">{c.description || 'No description'}</p>
                  ...
                  {c.status === 'rejected' && c.rejectionReason && (
                    <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-2 mb-3 text-xs text-red-600"><i className="ri-information-line mr-1"></i>{c.rejectionReason}</div>
                  )}

                  <div className="flex items-center gap-2 flex-wrap">
                    <button onClick={() => openDetail(c)} className="px-3 py-1.5 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded-lg text-sm font-medium hover:bg-purple-200 dark:hover:bg-purple-900/50 transition"><i className="ri-eye-line mr-1"></i>View</button>
                    <button onClick={() => openEdit(c)} className="px-3 py-1.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-lg text-sm font-medium hover:bg-blue-200 transition"><i className="ri-edit-line mr-1"></i>Edit</button>
                    {c.status === 'pending' && (
                      <>
                    ...
                    {c.status === 'active' && (
                      <button onClick={() => handleStatusChange(c.id, 'inactive')} className="px-3 py-1.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-lg text-sm font-medium hover:bg-gray-200 transition"><i className="ri-pause-line"></i></button>
                    )}
                    {c.status === 'inactive' && (
                      <button onClick={() => handleStatusChange(c.id, 'active')} className="px-3 py-1.5 bg-green-50 dark:bg-green-900/20 text-green-600 rounded-lg text-sm font-medium hover:bg-green-100 transition"><i className="ri-play-line"></i></button>
                    )}
                    {c.status === 'rejected' && (
                      <button onClick={() => handleApprove(c.id, c.title)} className="px-3 py-1.5 bg-green-50 dark:bg-green-900/20 text-green-600 rounded-lg text-sm font-medium hover:bg-green-100 transition"><i className="ri-check-line"></i></button>
                    )}
                    <button onClick={() => handleDelete(c.id, c.title)} className="px-3 py-1.5 bg-red-50 dark:bg-red-900/20 text-red-500 rounded-lg text-sm font-medium hover:bg-red-100 transition ml-auto"><i className="ri-delete-bin-line"></i></button>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b dark:border-gray-700">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">{editCourse ? 'Edit Course' : 'Create New Course'}</h2>
              <button onClick={() => setShowModal(false)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition"><i className="ri-close-line text-xl"></i></button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Course Title *</label>
                  <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} className="w-full px-4 py-2.5 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent" required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Course Code</label>
                  <input value={form.courseCode} onChange={e => setForm({ ...form, courseCode: e.target.value })} className="w-full px-4 py-2.5 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Category</label>
                  <input value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} placeholder="e.g. Computer Science" className="w-full px-4 py-2.5 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Type</label>
                  <select value={form.courseType} onChange={e => setForm({ ...form, courseType: e.target.value })} className="w-full px-4 py-2.5 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                ...
                <div>
                  ...
                  <select value={form.difficulty} onChange={e => setForm({ ...form, difficulty: e.target.value })} className="w-full px-4 py-2.5 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                    <option value="beginner">Beginner</option><option value="intermediate">Intermediate</option><option value="advanced">Advanced</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Semester</label>
                  <input value={form.semester} onChange={e => setForm({ ...form, semester: e.target.value })} className="w-full px-4 py-2.5 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Duration</label>
                  <input value={form.duration} onChange={e => setForm({ ...form, duration: e.target.value })} placeholder="e.g. 12 weeks" className="w-full px-4 py-2.5 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Max Students</label>
                  <input type="number" value={form.maxStudents} onChange={e => setForm({ ...form, maxStudents: parseInt(e.target.value) || 50 })} className="w-full px-4 py-2.5 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
                </div>
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Description</label>
                  <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} rows="3" className="w-full px-4 py-2.5 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent"></textarea>
                </div>
              </div>
              ...
              {!editCourse && (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Initial Subjects</label>
                    <button type="button" onClick={addSubjectRow} className="text-sm text-purple-600 hover:text-purple-800"><i className="ri-add-line mr-1"></i>Add Row</button>
                  </div>
                  {form.subjects.map((s, i) => (
                    <div key={i} className="flex items-center gap-2 mb-2">
                      <input value={s.title} onChange={e => updateSubjectRow(i, 'title', e.target.value)} placeholder="Subject title" className="flex-1 px-3 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
                      <input value={s.code} onChange={e => updateSubjectRow(i, 'code', e.target.value)} placeholder="Code" className="w-24 px-3 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
              ...
              <div className="flex justify-end gap-3 pt-4 border-t dark:border-gray-700">
                <button type="button" onClick={() => setShowModal(false)} className="px-5 py-2.5 border dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 font-medium">Cancel</button>
                <button type="submit" className="px-5 py-2.5 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition font-medium">{editCourse ? 'Update Course' : 'Create Course'}</button>
              </div>
            </form>
          </div>
        </div>
